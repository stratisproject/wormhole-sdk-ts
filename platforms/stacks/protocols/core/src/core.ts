...
export class StacksWormholeCore<N extends Network, C extends StacksChains>
  implements WormholeCore<N, C>
{
  ...
  async getGuardianSet(index: number): Promise<WormholeCore.GuardianSet> {
    ...
    const guardians = value['guardians'].value.map(
      (v: any) => v.value['compressed-public-key'].value,
    );
  ...
  async *verifyMessage(
    ...
    const res = await this.readonly('parse-and-verify-vaa', [
      Cl.buffer(serialize(vaa)),
    ]);
  ...
  async parseTransaction(txid: TxHash): Promise<StacksWormholeMessageId[]> {
    ...
    let retries = 0;
    while (
      (!data || !data?.tx_status || data?.tx_status === 'pending') &&
      retries < 10
    ) {
    ...
    const whEvent = events.filter((e: any) => {
      return (
        ...
        e.contract_log?.contract_id?.includes(
          StacksWormholeCore.STATE_CONTRACT_NAME,
        ) &&
        e.contract_log?.topic === 'print' &&
        e.contract_log?.value?.repr?.includes('post-message')
      );
    });
    const parsedEvents: any = whEvent.map((e: any) =>
      deserializeCV(e.contract_log?.value?.hex),
    );
    if (!parsedEvents) {
      return [];
    }
  ...
  private async getActiveCoreContract(): Promise<string> {
    const res = await this.readonly(
      'get-active-wormhole-core-contract',
      [],
      StacksWormholeCore.STATE_CONTRACT_NAME,
    );
    return cvToValue(res).value;
