...
import type { StacksUnsignedTransaction } from './unsignedTransaction.js';
...
export class StacksSigner<N extends Network, C extends StacksChains>
  implements SignAndSendSigner<N, C>
{
  ...
  constructor(
    ...
    this._address = privateKeyToAddress(
      this._privKey,
      this._network.toLowerCase() as StacksNetworkName,
    );
  }
    ...
    const txHashes = await Promise.all(
      signed.map(async (signedTx) => {
        ...
        if (txBroadcastResult.error) {
          ...
          throw new Error(
            `Failed to broadcast transaction: ${txBroadcastResult.error} ${txBroadcastResult.reason} - ${txBroadcastResult.txid} -`,
          );
        }
